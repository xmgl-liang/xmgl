...
func InitJobMgr() (err error) {
	...
	// 启动任务监听
	G_jobMgr.watchJobReplyLetter()
	G_jobMgr.watchDelFriend()
	G_jobMgr.watchAddNumFri()
...
// 监听任务变化
func (jobMgr *JobMgr) watchJobReplyLetter() (err error) {
	var (
	...
	// 1, get一下JOB_SAVE_REPLY_LETTER目录下的所有任务，并且获知当前集群的revision
	if getResp, err = jobMgr.kv.Get(context.TODO(), common.JOB_SAVE_REPLY_LETTER, clientv3.WithPrefix()); err != nil {
		return
	}
	...
	for _, kvpair = range getResp.Kvs {
		// 反序列化json得到Job
		if job, err = common.UnpackLetterJob(kvpair.Value); err == nil {
			// 同步给scheduler(调度协程)
	...
	go func() { // 监听协程
		...
		// 监听JOB_SAVE_REPLY_LETTER目录的后续变化
		watchChan = jobMgr.watcher.Watch(context.TODO(), common.JOB_SAVE_REPLY_LETTER, clientv3.WithRev(watchStartRevision), clientv3.WithPrefix())
		// 处理监听事件
		for watchResp = range watchChan {
			for _, watchEvent = range watchResp.Events {
				switch watchEvent.Type {
				case mvccpb.PUT: // 任务保存事件
					if job, err = common.UnpackLetterJob(watchEvent.Kv.Value); err != nil {
						continue
					}
...
// 监听任务变化
func (jobMgr *JobMgr) watchDelFriend() (err error) {
	var (
		getResp            *clientv3.GetResponse
		kvpair             *mvccpb.KeyValue
	...
	// 1, get一下JOB_DEL_FRIEND目录下的所有任务，并且获知当前集群的revision
	if getResp, err = jobMgr.kv.Get(context.TODO(), common.JOB_DEL_FRIEND, clientv3.WithPrefix()); err != nil {
		return
	}
	...
	for _, kvpair = range getResp.Kvs {
		// 反序列化json得到Job
		if job, err = common.UnpackDelFriend(kvpair.Value); err == nil {
			// 同步给scheduler(调度协程)
	...
	go func() { // 监听协程
		// 从GET时刻的后续版本开始监听变化
		watchStartRevision = getResp.Header.Revision + 1
		// 监听JOB_DEL_FRIEND目录的后续变化
		watchChan = jobMgr.watcher.Watch(context.TODO(), common.JOB_DEL_FRIEND, clientv3.WithRev(watchStartRevision), clientv3.WithPrefix())
		// 处理监听事件
		for watchResp = range watchChan {
			for _, watchEvent = range watchResp.Events {
				switch watchEvent.Type {
				case mvccpb.PUT: // 任务保存事件
					if job, err = common.UnpackDelFriend(watchEvent.Kv.Value); err != nil {
						continue
					}
				case mvccpb.DELETE: // 任务被删除了
				}
...
// 监听任务变化
func (jobMgr *JobMgr) watchAddNumFri() (err error) {
	var (
		getResp            *clientv3.GetResponse
		kvpair             *mvccpb.KeyValue
