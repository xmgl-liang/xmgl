...
// 监听任务变化
func (jobMgr *JobMgr) watchMailFriend() (err error) {
	var (
	...
	// 1, get一下JOB_FRIEND_MAIL目录下的所有任务，并且获知当前集群的revision
	if getResp, err = jobMgr.kv.Get(context.TODO(), common.JOB_FRIEND_MAIL, clientv3.WithPrefix()); err != nil {
		return
	}
	...
	for _, kvpair = range getResp.Kvs {
		// 反序列化json得到Job
		if job, err = common.UnpackMailFriend(kvpair.Value); err == nil {
			// 同步给scheduler(调度协程)
	...
	go func() { // 监听协程
		...
		// 监听JOB_FRIEND_MAIL目录的后续变化
		watchChan = jobMgr.watcher.Watch(context.TODO(), common.JOB_FRIEND_MAIL, clientv3.WithRev(watchStartRevision), clientv3.WithPrefix())
		// 处理监听事件
		for watchResp = range watchChan {
			for _, watchEvent = range watchResp.Events {
				switch watchEvent.Type {
				case mvccpb.PUT: // 任务保存事件
					if job, err = common.UnpackMailFriend(watchEvent.Kv.Value); err != nil {
						continue
					}
...
func (jobMgr *JobMgr) watchLetterByFri() (err error) {
	...
	// 1, get一下JOB_LETTER_MAIL目录下的所有任务，并且获知当前集群的revision
	if getResp, err = jobMgr.kv.Get(context.TODO(), common.JOB_LETTER_MAIL, clientv3.WithPrefix()); err != nil {
		return
	}
	...
	go func() { // 监听协程
		// 从GET时刻的后续版本开始监听变化
		watchStartRevision = getResp.Header.Revision + 1
		// 监听JOB_LETTER_MAIL目录的后续变化
		watchChan = jobMgr.watcher.Watch(context.TODO(), common.JOB_LETTER_MAIL, clientv3.WithRev(watchStartRevision), clientv3.WithPrefix())
		// 处理监听事件
		for watchResp = range watchChan {
			for _, watchEvent = range watchResp.Events {
				switch watchEvent.Type {
...
// 监听任务变化
func (jobMgr *JobMgr) watchNoLetter() (err error) {
	var (
		getResp            *clientv3.GetResponse
		kvpair             *mvccpb.KeyValue
		job                *common.NoLetter
	...
	// 1, get一下JOB_NO_LETTER_MAIL目录下的所有任务，并且获知当前集群的revision
	if getResp, err = jobMgr.kv.Get(context.TODO(), common.JOB_NO_LETTER_MAIL, clientv3.WithPrefix()); err != nil {
		return
	}
	...
	for _, kvpair = range getResp.Kvs {
		// 反序列化json得到Job
		if job, err = common.UnpackNoLetter(kvpair.Value); err == nil {
			// 同步给scheduler(调度协程)
	...
	go func() { // 监听协程
		// 从GET时刻的后续版本开始监听变化
		watchStartRevision = getResp.Header.Revision + 1
		// 监听JOB_NO_LETTER_MAIL目录的后续变化
		watchChan = jobMgr.watcher.Watch(context.TODO(), common.JOB_NO_LETTER_MAIL, clientv3.WithRev(watchStartRevision), clientv3.WithPrefix())
		// 处理监听事件
		for watchResp = range watchChan {
			for _, watchEvent = range watchResp.Events {
				switch watchEvent.Type {
				case mvccpb.PUT: // 任务保存事件
					if job, err = common.UnpackNoLetter(watchEvent.Kv.Value); err != nil {
						continue
					}
				case mvccpb.DELETE: // 任务被删除了
				}
